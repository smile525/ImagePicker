...
public class ImagePickerActivity extends BaseActivity implements ImagePickerAdapter.OnItemClickListener, ImageFoldersAdapter.OnImageFolderChangeListener {
    ...
    protected void getData() {
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU){
            boolean hasPermission = PermissionUtil.checkPermissionA14(this);
            if (!hasPermission) {
                ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.READ_MEDIA_IMAGES, Manifest.permission.READ_MEDIA_VIDEO,Manifest.permission.CAMERA}, REQUEST_PERMISSION_CAMERA_CODE);
            } else {
                startScannerTask();
            }
        }else {
        ...
        if (requestCode == REQUEST_PERMISSION_CAMERA_CODE) {
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU){
                if (grantResults.length >= 1) {
                    int imageResult = grantResults[0];//图片权限
                    int videoResult = grantResults[1];//视频权限
                    int cameraResult = grantResults[2];//相机权限
                    boolean imageGranted = imageResult == PackageManager.PERMISSION_GRANTED;//图片权限
                    boolean videoGranted = videoResult == PackageManager.PERMISSION_GRANTED;//视频权限
                    boolean cameraGranted = cameraResult == PackageManager.PERMISSION_GRANTED;//相机权限
                    if (imageGranted && videoGranted && cameraGranted) {
                        //Android 13+ 具有图片，视频，拍照权限
                        startScannerTask();
                    } else {
                        //没有权限
                        Toast.makeText(this, getString(R.string.permission_tip14), Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }
            }else {
